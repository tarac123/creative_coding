class PieChart {
    constructor(obj) {
        ...
        this.outerLabels = obj.outerLabels || [];
        this.innerLabels = obj.innerLabels || [];
    ...
    renderPie() {
        ...
        for (let i = 0; i < this.outerData.length; i++) {
            let angle = map(this.outerData[i], 0, totalOuter, 0, 360);
            fill(this.outerColors[i]);
            arc(0, 0, this.chartHeight, this.chartHeight, lastAngle, lastAngle + angle, PIE);
            lastAngle += angle;
        }
        ...
        for (let i = 0; i < this.innerData.length; i++) {
            ...
            arc(0, 0, this.chartHeight * 0.6, this.chartHeight * 0.6, lastAngle, lastAngle + angle, PIE);
            lastAngle += angle;
        }
        pop();
    }
    ...
    renderLegend() {
        push();
        const legendX = this.chartPosX + this.chartHeight / 2 + 20;
        const legendY = this.chartPosY - this.chartHeight / 3;
        const rectSize = 15;
        const lineHeight = 25;
        ...
        const innerLegendY = legendY + (this.outerLabels.length + 1) * lineHeight;
        fill(0);
        textStyle(BOLD);
